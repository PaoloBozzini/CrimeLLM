...
| `synthesize.py` | synthesize | `Synthesizer` ABC. Backends: `AnthropicSynthesizer`, `OllamaSynthesizer`, `AirLLMSynthesizer`, `FakeSynthesizer`. `extract_citations()` + `check_citations()` enforce that every cited id appears in the retrieved set. Disclaimer / system prompt / user-prompt scaffold / fabrication-warning prefix all language-routed via `Query.language` |
| `prompts.py` | synthesize | EN + DA system prompts (common-law vs civil-law tone), disclaimers, no-context messages, and `format_human_citation` helpers that enrich the prompt context block per jurisdiction (DK lbk → short title + §, EU CELEX → `Reg (EU) YYYY/N, Art. X`, **DK ECLI cases → caption + parallel Ufr `U.YYYY.NNNNH` + ECLI when `Case.citations` carries an Ufr alt-id**) — without touching the bracketed canonical id the citation guard checks |
