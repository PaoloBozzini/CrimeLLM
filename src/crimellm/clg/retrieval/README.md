...
| `prompts.py` | synthesize | EN + DA system prompts (common-law vs civil-law tone), disclaimers, no-context messages, and `format_human_citation` helpers that enrich the prompt context block per jurisdiction (DK lbk → short title + §, EU CELEX → `Reg (EU) YYYY/N, Art. X`) — without touching the bracketed canonical id the citation guard checks |
